 ...
 *
 * One row per sellable resource (Wood/Fish/Grain — Gold isn't
 * sellable for itself), each with three buttons: sell 1 / sell 10 /
 * sell all. A Close button dismisses the screen; a click anywhere
 * else outside the panel also closes it, matching the menu
...
#define TRADE_ROW_H       60
#define TRADE_ROW_PAD     10
#define TRADE_MARGIN      20
#define TRADE_BTN_W       72
#define TRADE_BTN_H       30
#define TRADE_BTN_GAP     10
#define TRADE_CLOSE_H     36

/* Sellable resources: RES_WOOD, RES_FISH, RES_GRAIN. RES_GOLD is
 * excluded — you can't sell currency for itself — and conveniently
...
#define TRADE_H (TRADE_TITLE_H + TRADE_SELLABLE_COUNT * (TRADE_ROW_H + TRADE_ROW_PAD) \
 ...
 * display of current amounts and prices); mouse_x/y highlight the
 * hovered button. */
void trade_ui_draw(SDL_Renderer *renderer, int screen_w, int screen_h,
                   const Stockpile *s, int mouse_x, int mouse_y);

/* Hit-test a click against the trade overlay. On TRADE_HIT_SELL,
 * *out_res is the resource to sell and *out_qty is how many units
 * (-1 means "sell all currently in stock" — the caller resolves
 * that against the live stockpile, since this function doesn't see
 * it). */
