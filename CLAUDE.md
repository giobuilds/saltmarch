...
- `people_view.c/h` + `people_ui.c/h` — the people (`P`): the island's wellbeing score, the six factors with their weights (the two unmodelled ones listed and marked, never omitted), and the cast, each row carrying its own six factors as a sparkline. Read-only — the only click that does anything closes it, which is why it needs no `UiState` field and nothing in the recorded intent stream
...
docs/README.md's "Phase N" sections are a changelog of major feature additions (procedural gen → building placement → resource ticks → population → sprites → sprite removal/zoom) written in past commits — read it before assuming a feature (like sprites) is still in the current build; check `CMakeLists.txt`'s `SALTMARCH_SIM_SOURCES` / `SALTMARCH_CLIENT_SOURCES` lists against `src/` to see what's actually compiled. docs/MMO_PLAN.md's phases are all done (1–6, plus every "later phase"), and its closing section lists what is deliberately not built yet — authentication, account tooling, sharding, and SERVER_AUTHORITY's optional Phase 5; docs/SUPPLY_CHAIN.md's eight phases are all done; docs/SERVER.md covers the dedicated server, including the transport hardening and the snapshot format that bounds join cost; docs/MARITIME_PLAN.md is built (order book, routes and charts, hulls and pirates) and docs/SERVER_AUTHORITY.md is built through Phase 3 (state pushes, local prediction, per-client redaction) with only its optional Phase 5 left; docs/AUTH_PLAN.md is built through Phase 2 (`account.c`/`sha256.c` server-side, `config.c` client-side, `saltmarch_host --accounts`) with TLS and passwords still design; docs/NEEDS_PLAN.md is complete (basics vs luxuries, the happiness ladder, per-resident consumption, and the closure guard in `tests/test_closure.c` that keeps a base tier staffable); docs/LIFE_PLAN.md is built through Phase 7 (multi-worker buildings, residents as named individuals, the calendar, ageing and death, households of ten founded from a per-island settler allowance and grown only by birth, a reserve of unhoused couples who emigrate if nobody roofs them, and gold that enters the world as taxed wages rather than being minted by housing) complete. Phase 9 is `wellbeing.c/h` in the SDL-free UI library: a read-only six-factor projection over the UI snapshot, nothing hashed, with two factors (corruption, generosity) marked unmodelled and their weight excluded rather than contributed. The snapshot carries a ten-resident **cast** rather than a census. Phase 9b is its screen (`people_view.c/h` + `people_ui.c/h`, `P`), which found the projection scoring empty ground 2.8/10 on its first frame — income is a constant, so with nobody there that one term carried the whole weighted sum; `wellbeing_island` now guards on population. Phase 8 gives every resident an integer productivity in a band of 85-140 from four independent inputs (prime, married, fed, tenure), and production advances by the crew's condition as well as its headcount — note `Building.timer` and the production period are both scaled by `PRODUCTIVITY_BASE` so the percentage never divides, which is what stops a lone hungry worker producing exactly nothing. Phase 7b lowered the working age to twelve (adulthood, marriage and childbearing stay at eighteen) and made a house a LINE rather than a tenancy — the eldest child born there inherits it when the elders die — which doubled the working share to ~24% and put the base tier back under the wall at 0.98. **Two things remain unmeasured**: whether a fully-built island funds its own imports (the affordability probe builds a deliberately import-dependent village and is the wrong fixture for it), and — until the fixture was lengthened — anything demographic at all in the determinism gate. `replay_record_demo_session` now runs fifty years (`DEMO_SESSION_TICKS`) over two houses, a hut and a farm, and asserts the village raised, married and employed its own children; it costs about seven seconds; its calendar decision was settled at Phase 4 — the needs tick is the month; docs/VISIBILITY.md and docs/PRIVACY.md are still design notes — what a client is allowed to know, and personal data. docs/UI_PLAN.md is complete: every screen the sim needs now exists. docs/ARCHITECTURE.md explains how the pieces fit.
